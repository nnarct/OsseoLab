...
def add_case_surgeon():
    try:
        ...
        existing = CaseSurgeon.query.filter_by(case_id=case_id, surgeon_id=doctor_id).first()
        if existing:
...
def get_surgeons_by_case(case_id):
    try:
        surgeons = CaseSurgeon.query.filter_by(case_id=case_id).order_by(CaseSurgeon.created_at).all()
        result = []
        for s in surgeons:
            doctor = s.surgeon
